...
- `dashboard.html` — la page publiée comme Artifact, statique (partagée
  publiquement — voir ADR-0003). Contient les données intégrées dans un
  bloc `<script type="application/json">` : un Artifact est un fichier
  unique, il ne peut pas aller chercher `data/matches.json` par un `fetch`.
